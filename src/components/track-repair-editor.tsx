...
function RepairMap({ existing, controls, preview, onAdd, onMove }: { existing: StoredPoint[]; controls: Coordinate[]; preview: ReconstructedPoint[]; onAdd(point: Coordinate): void; onMove(index: number, point: Coordinate): void }) {
  ...
  useEffect(() => {
    ...
    void import("leaflet").then((L) => {
      ...
      const markerStep = Math.max(1, Math.ceil(existing.length / 250));
      existing.filter((_, index) => index % markerStep === 0 || index === existing.length - 1).forEach((p) => L.circleMarker([p.lat, p.lng], { radius: 3, color: p.source === "manual-reconstruction" ? "#009bc4" : "#071a2b", weight: 1, fillOpacity: .8 }).bindTooltip(`${new Date(p.received_at).toLocaleString()} · ${p.source}`).addTo(layer));
  ...
  useEffect(() => {
    let active = true;
    if (!editLayer.current) return; const layer = editLayer.current;
    void import("leaflet").then((L) => {
      if (!active) return;
      layer.clearLayers();
